...
const ViewPayment = ({
  ...
  useEffect(() => {
    setSupplier(
      allSupplier?.filter((i) => i?._id === singlePurchase?.supplier)?.[0]
    );
    setWarehouse(
      allWarehouse?.filter((i) => i?._id === singlePurchase?.wh)?.[0]
    );
  }, [singlePurchase]);
  return (
    <Modal
      ...
      footer={[
        <div>
          <Button key="cancel" onClick={() => setViewPaymentModal(false)}>
            Cancel
          </Button>
